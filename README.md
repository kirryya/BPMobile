Cверстана полноэкранная веб-страница (баннер) для мобильных устройств iPhone SE, 8+, X, 14, все в портретной ориентации, на 7 языках (языковые строки в папке проекта). Язык выбирается исходя из языка системы, а также осуществляется переключение путем передачи параметра 'lang' в строке запроса (?lang=en).
